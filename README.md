...
场景名称/描述里出现雨、雷、震、塌、厮杀等字眼时，阅读界面自动进入对应氛围（雨夜下雨、动荡场景纸面震抖）。**判定与渲染都在前端，不改任何接口**：

- **判定**：[`novel_game/static/app.js`](novel_game/static/app.js) 的 `detectSceneFx` 用一张关键词表（`rain` / `shake`）扫「场景名 + 场景描述」，首个命中即生效；无命中回到静景。命中结果写进 `body[data-scene-fx]`，**不进 state、不落盘**。
- **雨**：`.fx` 层里两层斜向条纹（远细慢、近粗快），速度差形成纵深。条纹位移取各自周期的**整数倍**，因此循环衔接处没有可见跳变。
- **震抖**：`.stream`（整张纸）入场抖一次即按 `ease-out` 衰减停下，**不做持续抖动**，不干扰阅读。
- **性能与可访问性**：全部是合成层 `transform` 动画（不触发布局/重绘）；`.fx` 与静景回合都不产生动画——`body` 上没有该属性时所有规则都不生效，零开销。系统开启「减少动效」时雨丝直接不显示、震抖禁用。
